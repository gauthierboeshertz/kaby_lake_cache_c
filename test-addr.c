...
START_TEST(test_init_virt_addr) {
    ...
    REPEAT(100) {
        ...
        virt_addr_t vaddr;
        uint64_t pgd = generate_random_size_bit(16);
        uint64_t pud = generate_random_size_bit(16);
        uint64_t pmd = generate_random_size_bit(16);
        uint64_t pte = generate_random_size_bit(16);
        uint64_t page_offset = generate_random_size_bit(16);
...
START_TEST(virt_addr_t_to_virtual_page_number_test) {
    ...
    REPEAT(100) {
        ...
        uint16_t pgd = generate_random_size_bit(9);
        uint16_t pud = generate_random_size_bit(9);
        uint16_t pmd = generate_random_size_bit(9);
        uint16_t pte = generate_random_size_bit(9);
        ...
        virt_addr_t final;
        init_virt_addr64(&final, vaddr1 << 12);
...
START_TEST(virt_addr_to_uint64) {
    ...
    REPEAT(100) {
        ...
        uint16_t pmd = generate_random_size_bit(9);
        uint16_t pte = generate_random_size_bit(9);
        uint16_t off = generate_random_size_bit(12);
        ...
        ck_assert_int_eq((test >> (PAGE_OFFSET + 9))& mask9 , vaddr.pmd_entry);
        ck_assert_int_eq( (test >> (PAGE_OFFSET + 9 * 2))& mask9, vaddr.pud_entry);
        ck_assert_int_eq( (test >>  (PAGE_OFFSET + 9 * 3))& mask9 , vaddr.pgd_entry);
...
START_TEST(init_phy_addr_test) {
    ...
    REPEAT(100) {
        ...
        uint32_t pagenum = generate_Nbit_random(32);
        uint32_t off = generate_Nbit_random(12);
